...
const SupportSection = () => {
  ...
  const handleCall = (value: string | undefined) => {
    const ii = contactItems.find((el) => el.value === value);
    if (ii?.id === 1) {
      window.open(`tel:${ii.value}`);
    } else if (ii?.id === 2) {
      window.open(`https://wa.me/${ii.value}?text=${messages.cil}`);
    } else if (ii?.id === 3) {
      window.open(`https://wa.me/${ii.value}?text=${messages.closingForm}`);
    } else if (ii?.id === 4) {
      window.open(`https://wa.me/${ii.value}?text=${messages.contract}`);
    } else if (ii?.id === 5) {
      window.open(`https://wa.me/${ii.value}?text=${messages.coverage}`);
    }
  };
  return (
    <section
      className="flex flex-col h-[70vmin] bg-[#f3f2f1] items-center py-10 gap-20"
    ...
    >
      ...
      <div className="flex items-center justify-center gap-8">
        {contactItems.map((item, index) => (
          <motion.button
            key={index}
            className="flex flex-col space-y-6 w-full h-full items-center border border-primary hover:bg-primary/50 hover:text-white hover:border bg-primary/30 p-16 cursor-pointer rounded-lg"
            variants={itemVariants}
            initial="hidden"
            whileInView="visible"
          ...
          >
            <div className="bg-white w-[80px] h-[80px] flex items-center justify-center rounded-full">
              <ImageIcon src={item.icon} size={50} alt={item.label} />
            </div>
